class Day1 : Fixture
{
    int Solve1(string input) => Enumerable
        .Zip(Parse(input, 1).Ordered(), Parse(input, 2).Ordered())
        .Sum(v => Math.Abs(v.First - v.Second));

    int Solve2(string input) => Extensions
        .GroupJoin(Parse(input, 1), Parse(input, 2), (l, r) => l * r.Count())
        .Sum();
